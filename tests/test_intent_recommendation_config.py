...
def test_intent_recommendation_from_config():
    ...
    result = form.process_input('我要做视频剪辑', llm, semantic_mapper)
    ...
    if chip_slot and chip_slot.status == SlotStatus.FILLED:
        ...
        assert chip_slot.value.value == 'M3 Pro', f"期望M3 Pro，实际: {chip_slot.value.value}"
    ...
    if storage_slot and storage_slot.status == SlotStatus.FILLED:
        ...
        assert storage_slot.value.value == '1TB', f"期望1TB，实际: {storage_slot.value.value}"
    ...
    if size_slot3 and size_slot3.status == SlotStatus.FILLED:
        ...
        assert size_slot3.value.value == '13寸'
